...
def get_train_pipelines(mean, std):
    if MMCompose is None or rand_range_aug is None:
        raise RuntimeError("mmcls is required for clip-strong_2 transforms.")
...
def get_data_transforms(transforms_type: str, 
    ...
    if 'clip' in transforms_type:
        ...
        clip_data_transforms["strong_2"] = (
            MMCompose(get_train_pipelines(pixel_mean, pixel_std))
            if MMCompose is not None
            else clip_data_transforms["strong_1"]
        )
