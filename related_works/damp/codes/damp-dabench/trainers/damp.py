...
from dabench_data import build_dabench_data_bundle, describe_dataset_source
...
def _load_openai_clip_checkpoint(path: Path):
    try:
        model = torch.jit.load(str(path), map_location="cpu").eval()
        return clip.build_model(model.state_dict())
    except RuntimeError:
...
def load_clip_to_cpu(cfg):
    ...
    local_path = getattr(cfg.MODEL.BACKBONE, "LOCAL_PATH", "")
    if local_path:
        resolved = Path(local_path).expanduser().resolve()
        if not resolved.is_file():
            raise FileNotFoundError(f"Configured CLIP local checkpoint not found: {resolved}")
        print(f"Loading CLIP from local checkpoint: {resolved}")
        return _load_openai_clip_checkpoint(resolved)

    url = clip._MODELS[backbone_name]
    model_path = clip._download(url, cfg.MODEL.BACKBONE.PATH)
    print(f"Loading CLIP via original download path: {model_path}")
    return _load_openai_clip_checkpoint(Path(model_path))
...
class DAMP(TrainerXU):
    ...
    def build_data_loader(self):
        ...
        bundle = build_dabench_data_bundle(
            cfg,
            weak_transform=tfm_train,
            strong_transform=tfm_train_strong,
            eval_transform=tfm_test,
        )
        self.dm = bundle
        self.train_loader_x = bundle.train_loader_x
        self.train_loader_u = bundle.train_loader_u
        self.val_loader = bundle.val_loader
        self.test_loader = bundle.test_loader
        self.num_classes = bundle.num_classes
        self.lab2cname = bundle.lab2cname
        print(f"Using dabench data source: {describe_dataset_source(cfg)}")
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if prec == "amp":
            with autocast():
                ...
                loss_u = zero if mask_ge.sum() == 0 else (F.cross_entropy(output_u, label_p, reduction='none') * mask_ge).sum() / mask_ge.sum()
                loss_u2 = zero if mask_ge.sum() == 0 else (F.cross_entropy(output_u2, label_p, reduction='none') * mask_ge).sum() / mask_ge.sum()
            ...
            mix_lambda = self.epoch / self.max_epoch
            pseudo_label = (
                torch.softmax(output_u.reshape(-1, self.n_cls), dim=-1) * mix_lambda
                + torch.softmax(pseudo_label_logits.reshape(-1, self.n_cls), dim=-1) * (1 - mix_lambda)
            ).detach()
            ...
            x_ind_label = torch.arange(self.cfg.DATALOADER.TRAIN_X.BATCH_SIZE, dtype=torch.long, device=self.device)
            loss_x_ind = (F.cross_entropy(output_x_ind, x_ind_label) + F.cross_entropy(output_x_ind.permute(1, 0), x_ind_label)) / 2.0

            u_ind_label = torch.arange(self.cfg.DATALOADER.TRAIN_U.BATCH_SIZE, dtype=torch.long, device=self.device)
            loss_u_ind = (F.cross_entropy(output_u_ind, u_ind_label) + F.cross_entropy(output_u_ind.permute(1, 0), u_ind_label)) / 2.0
