...
This directory is a `dabench`-backed reproduction scaffold for DAMP. The trainer and loss structure stay close to the original code, but dataset loading is routed through local `dabench` prepared data instead of Dassl dataset registration.
...
For this variant, dataset locations are resolved by `dabench` config rather than shell-script paths.

- Office-Home / VisDA-2017 use the prepared paths configured in `dabench`.
- miniDomainNet uses `dabench` dataset id `minidomainnet`, which reuses prepared `domainnet` data plus a `splits_mini/` directory configured in `dabench`.
- The `--root` argument in scripts is kept only for CLI compatibility.
