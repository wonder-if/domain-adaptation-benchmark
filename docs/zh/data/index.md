...
| iWildCam | 🤗 [anngrosha/iWildCam2020](https://huggingface.co/datasets/anngrosha/iWildCam2020) | 相机陷阱 / `location` id；325 train，91 test |
...
域过滤使用 manifest 和 prepared 数据里明确记录的 domain 值：
...
如果只想读取一个明确的 domain/split view，可以使用 `load_view`。它仍然返回底层 dataset 对象，不会额外封装：
...
如果训练循环需要 PyTorch 风格的 dataset，可以显式请求 torch view：
